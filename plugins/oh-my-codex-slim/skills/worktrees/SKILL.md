---
name: worktrees
description: Manage Git worktrees as OMC safe isolated coding lanes for complex, risky, or parallel work.
---

# Worktrees Orchestration Protocol

The `worktrees` skill provides an opinionated, safe orchestration protocol for
managing Git worktrees as isolated coding lanes. Its value is giving the Orchestrator a consistent OMC workflow for parallel agents, risky experiments, integration review, and cleanup.
...
- Ensure the branch name (e.g. `omcs/<slug>` or custom project convention) does not already exist locally or on remote.
...
```gitignore
# BEGIN oh-my-codex-slim worktrees
.slim/worktrees/
.slim/worktrees.json
# END oh-my-codex-slim worktrees
```
...
2. Formulate a branch name. Default to `omcs/<slug>` unless project/user conventions dictate otherwise.
