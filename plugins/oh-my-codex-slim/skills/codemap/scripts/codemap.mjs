...
// Resolve symlinks on both sides so direct invocation is detected even when the
// script lives under a symlinked path (e.g. macOS /var -> /private/var, or a
// symlinked plugin cache). Falls back to the non-realpath value if it cannot resolve.
...
if (process.argv[1] && realOrSelf(process.argv[1]) === realOrSelf(currentFilePath)) {
  process.exit(main());
}
