...
- improve their oh-my-codex-slim setup based on actual usage;
...
- explain restart requirements for Codex config, prompt, agent, skill, MCP, or
  plugin changes.
...
2. Project-local guidance and memories, such as `AGENTS.md`, `.codex/`, `$CODEX_HOME`,
...
3. Existing skills, commands, agents, prompt overrides, MCP permissions, and
   oh-my-codex-slim configuration.
4. Recent Codex logs or session artifacts if they are available and safe to
   inspect.
...
- When config, prompt, agent, skill, MCP, or plugin files change, tell the user:
  "This should apply on the next Codex run; restart Codex if you need it
  immediately."
